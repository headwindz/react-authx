...
import { AuthProvider, AuthConsumer, setCustomValidator } from '../../src';
...
describe('AuthConsumer should work with children node', () => {
  test('should work if authKey matches', () => {
    const element = dumpElement(<AuthConsumer authKey="code1">{div}</AuthConsumer>);
    render(element);
    expect(screen.queryByText(TEXT)).not.toBeNull();
  });

  test('should work for multiple auth keys', () => {
    const element = dumpElement(<AuthConsumer authKey="abc,code1">{div}</AuthConsumer>);
    render(element);
    expect(screen.queryByText(TEXT)).not.toBeNull();
  });

  test('should work if no authKey matches', () => {
    const element = dumpElement(<AuthConsumer authKey="code100">{div}</AuthConsumer>);
    render(element);
    expect(screen.queryByText(TEXT)).toBeNull();
  });

  test('should work for multiple auth keys', () => {
    const element = dumpElement(<AuthConsumer authKey="code10,code200">{div}</AuthConsumer>);
    render(element);
    expect(screen.queryByText(TEXT)).toBeNull();
  });
});
...
describe('AuthConsumer should work with children render props', () => {
  test('should work if authKey matches', () => {
    const element = dumpElement(
      <AuthConsumer authKey="code1">{(hasAuth) => (hasAuth ? div : noAuthDiv)}</AuthConsumer>,
    );
    render(element);
    expect(screen.queryByText(TEXT)).not.toBeNull();
  ...
  test('should work if authKey does NOT match', () => {
    const element = dumpElement(
      <AuthConsumer authKey="code100">{(hasAuth) => (hasAuth ? div : noAuthDiv)}</AuthConsumer>,
    );
    render(element);
    expect(screen.queryByText(TEXT)).toBeNull();
...
describe('hasAuth should work with custom setting', () => {
  ...
  test('should work with custom validator', () => {
    const element = dumpElement(<AuthConsumer authKey={SPECIFIC_KEY}>{div}</AuthConsumer>);
    render(element);
    expect(screen.queryByText(TEXT)).not.toBeNull();
  });
});
